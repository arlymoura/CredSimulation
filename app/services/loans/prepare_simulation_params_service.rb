module Loans
  class PrepareSimulationParamsService < ::ApplicationService
    ...
    def call
      ...
      return failure('Invalid loan amount') if normalize_loan_amount_valid?(normalized_loan_amount_cents)

      normalized_birth_date = normalize_birth_date
      return failure('Invalid birth date') if normalize_birth_date_valid?(normalized_birth_date)

      age = calculate_age(normalized_birth_date)
      return failure('Invalid birth date') if age <= 0

      normalized_term = @term_in_months.to_i
      return failure('Invalid term') if normalized_term <= 0
    ...
    def normalize_loan_amount_valid?(normalized_loan_amount_cents)
    ...
    def normalize_birth_date_valid?(normalized_birth_date)
